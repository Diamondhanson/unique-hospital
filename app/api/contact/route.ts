...
const ContactSchema = z.object({
  ...
  phone: z.string().trim().regex(phoneRegex).max(40).optional().or(z.literal("")),
...
function renderEmail({ fullName, email, phone, subject, message, locale }: EmailFields) {
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,0.08);">
            <tr>
              <td style="background:linear-gradient(120deg,#0070d2 0%,#1aa0c9 55%,#7bc043 100%);padding:28px 32px;color:#ffffff;">
                ...
                <div style="font-size:22px;font-weight:600;margin-top:6px;line-height:1.3;">New contact form submission</div>
              </td>
            </tr>
            <tr>
              <td style="padding:28px 32px 8px 32px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  ...
                  ${phone ? row("Phone", `<a href="tel:${escape(phone)}" style="color:#0070d2;text-decoration:none;">${escape(phone)}</a>`) : ""}
                  ${row("Subject", escape(subject))}
            ...
            <tr>
              <td style="padding:8px 32px 24px 32px;">
                <div style="font-size:12px;letter-spacing:0.06em;text-transform:uppercase;color:#64748b;font-weight:600;margin-bottom:8px;">Message</div>
                <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:16px 18px;font-size:15px;line-height:1.55;color:#0f172a;">${messageHtml}</div>
              </td>
            </tr>
            <tr>
              <td style="padding:0 32px 28px 32px;">
                <a href="mailto:${escape(email)}?subject=${encodeURIComponent("Re: " + subject)}" style="display:inline-block;background:linear-gradient(120deg,#0070d2 0%,#1aa0c9 55%,#7bc043 100%);color:#ffffff;text-decoration:none;font-weight:600;font-size:14px;padding:12px 22px;border-radius:999px;">Reply to ${escape(fullName.split(" ")[0])}</a>
              </td>
            </tr>
            <tr>
